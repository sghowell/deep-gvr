...
5. If Tier 3 is expected, inspect `verification.tier3.backend` in the runtime config first. For `backend: aristotle`, run `bash scripts/setup_mcp.sh --install --check` so `~/.hermes/config.yaml` has `mcp_servers.aristotle` and the local environment confirms `ARISTOTLE_API_KEY` plus Hermes MCP readiness. For `backend: mathcode`, ensure the configured local checkout exists and includes `AUTOLEAN/`, `lean-workspace/`, and an executable `run` wrapper. For `backend: opengauss`, do not claim readiness from the raw checkout alone; run `uv run python scripts/diagnose_opengauss.py --json` and require the installed `gauss` runtime plus `~/.gauss/config.yaml` before treating the shipped OpenGauss backend as ready in that operator environment.
...
- The orchestrator mediates Tier 3 as verifier -> selected formal backend -> verifier, persisting the formal request, lifecycle state when applicable, transport trace, and returned results under the session artifacts directory.
- Aristotle remains the primary MCP-backed Tier 3 path, while MathCode and OpenGauss are available as bounded local CLI backends for mathematical proof obligations.
- OpenGauss diagnostics now have a dedicated readiness command at `scripts/diagnose_opengauss.py`; use it to distinguish raw-checkout issues from installed-runtime readiness for the shipped bounded local CLI backend.
