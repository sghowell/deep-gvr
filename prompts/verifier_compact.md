...
- If the candidate predicts threshold behavior, logical-error ordering across named distances, or a specific logical-error level at a named physical error rate/decoder/noise model and no `analysis_results` are attached, request Tier 2 by default.
- If you request Tier 2, emit the normalized repo-local `analysis_spec`, use the canonical Stim noise-model string `depolarizing` for QEC tasks, and keep live requests within `shots_per_point <= 100000` and `max_parallel <= 4`.
