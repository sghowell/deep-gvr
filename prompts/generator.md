...
- For Tier-2-driven quantitative claims, prefer the smallest falsifiable prediction the selected analysis adapter can actually check; do not invent absolute magnitudes or prefactors unless a cited source directly supports them.
...
- For literature-grounded threshold-understanding questions, keep the hypothesis literature-backed and scoped to threshold existence or cited threshold regimes; do not restate below-threshold suppression as a fresh analysis-ready prediction unless the prompt explicitly asks for empirical validation.
- For literature-grounded threshold-understanding questions, keep `expected_results` on threshold existence, regime separation, or cited threshold ranges; do not introduce fresh distance-ordering or threshold-scan simulation targets unless the prompt explicitly asks for empirical validation.
- For algorithmic scaling questions, keep the candidate on the asymptotic decode complexity the prompt actually asks for; do not add threshold comparisons, empirical runtime targets, or per-operation worst-case claims unless the prompt explicitly asks for them.
- For pure counting or asymptotic cost questions, prefer one asymptotic statement plus at most one concrete formula or worked example; do not compare multiple layouts or enumerate multiple exact sequences unless the prompt explicitly asks for that comparison.
- For compact theorem or asymptotic proof claims, keep `expected_results` on the proof statement or derived asymptotic consequence; do not add empirical simulation targets unless the prompt explicitly asks for simulation.
- For small-distance analysis-backed claims, keep the hypothesis on the ordering the prompt actually asks for, such as `p_L(d=5) < p_L(d=3)`; do not upgrade it to exponential scaling, per-step suppression ratios, or fit-quality claims unless the prompt explicitly asks for those stronger statements.
- In `expected_results`, prefer direct checks over ratio targets or fit-quality claims when the analysis budget may leave evidence sparse.
