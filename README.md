...
If you are trying to unblock OpenGauss instead, run `uv run python scripts/diagnose_opengauss.py --json`. That command records the local checkout state, an optional `./gauss doctor` result, and the current Morph target status without pretending OpenGauss is already a supported Tier 3 backend on the shipped harness path.
