...
- Aristotle remains the primary MCP-backed Tier 3 path, while MathCode is available as a local CLI backend for mathematical proof obligations.
- OpenGauss diagnostics now have a dedicated blocked-state command at `scripts/diagnose_opengauss.py`, but the actual backend remains blocked externally until plan 31 can resume.
