...
from deep_gvr.release_surface import collect_release_preflight


def _print_human_report(report: dict[str, object]) -> None:
    print(f"Release preflight for {report['skill_name']} {report['version']}")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run deep-gvr release-surface preflight checks")
    parser.add_argument("--json", action="store_true", help="Emit JSON instead of human-readable text")
    parser.add_argument(
        "--operator",
        action="store_true",
        help="Exit non-zero unless the configured operator runtime is fully ready, not just structurally release-complete.",
    )
    parser.add_argument(
        "--config",
        type=Path,
