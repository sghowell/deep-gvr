...
| opengauss-formal-backend | blocked_external | OpenGauss is supported as the interactive Lean backend alongside Aristotle. | The runtime still has no selector or transport for OpenGauss, and the current upstream local installer path fails before Gauss can be installed locally: both the script default Morph target and the README-pinned target currently return `404` from `morph.new`. | A valid upstream OpenGauss install/distribution path plus backend contracts, transport code, and operator workflow. | [31-opengauss-formal-backend.md](../plans/31-opengauss-formal-backend.md) | Formal backend selection supports OpenGauss, docs describe the operator flow, benchmark coverage includes an OpenGauss-backed case, and the local installer produces a working `gauss` runtime again. |
| mathcode-formal-backend | planned | MathCode is supported as an additional local Lean formalization backend alongside Aristotle and OpenGauss. | The architecture and runtime currently do not expose MathCode as a Tier 3 backend, even though a local MathCode CLI path is available to integrate. | Backend contracts, transport code, operator workflow, and benchmark coverage for MathCode. | [35-mathcode-formal-backend.md](../plans/35-mathcode-formal-backend.md) | Formal backend selection supports MathCode, evidence artifacts identify MathCode-backed proof attempts, and benchmark coverage includes at least one MathCode-backed formal case. |
