...
from .contracts import CodexReviewQaCatalog, CodexReviewQaSpec
from .json_schema import SchemaValidationError, validate

REPO_ROOT_PLACEHOLDER = "__DEEP_GVR_REPO_ROOT__"
_SSH_DEVBOX_CATALOG_PATH = Path("codex_ssh_devbox/catalog.json")
...
def ssh_devbox_catalog_path(root: Path | None = None) -> Path:
    return (root or repo_root()) / _SSH_DEVBOX_CATALOG_PATH
...
def expected_codex_ssh_devbox_catalog(root: Path | None = None) -> CodexReviewQaCatalog:
    effective_root = root or repo_root()
    payload = json.loads(
        (effective_root / "templates" / "codex_ssh_devbox_catalog.template.json").read_text(encoding="utf-8")
    )
    payload["version"] = _project_version(effective_root)
    return CodexReviewQaCatalog.from_dict(payload)


def codex_ssh_devbox_surface_errors(root: Path | None = None) -> list[str]:
    effective_root = root or repo_root()
    errors: list[str] = []
    catalog_path = ssh_devbox_catalog_path(effective_root)
    schema = json.loads(_schema_path("codex_ssh_devbox_catalog.schema.json", effective_root).read_text(encoding="utf-8"))

    if not catalog_path.exists():
        return [f"{catalog_path.relative_to(effective_root)}: required Codex SSH/devbox catalog is missing"]
    ...
    try:
        ...
        return [f"{catalog_path.relative_to(effective_root)}: invalid Codex SSH/devbox catalog: {exc}"]

    expected_catalog = expected_codex_ssh_devbox_catalog(effective_root)
    if actual_catalog.to_dict() != expected_catalog.to_dict():
        errors.append(
            f"{catalog_path.relative_to(effective_root)}: Codex SSH/devbox catalog is out of sync with repo metadata"
        )

    for spec in expected_catalog.templates:
        template_path = effective_root / spec.template_path
        if not template_path.exists():
            errors.append(f"{template_path.relative_to(effective_root)}: required Codex SSH/devbox template is missing")
            continue
        actual_template = template_path.read_text(encoding="utf-8")
        if actual_template != spec.prompt:
            errors.append(
                f"{template_path.relative_to(effective_root)}: SSH/devbox template is out of sync with the catalog"
            )
    return errors


def export_codex_ssh_devbox_bundle(
    output_root: Path,
    *,
    root: Path | None = None,
    force: bool = False,
    ...
    catalog = expected_codex_ssh_devbox_catalog(effective_root)
    exported_paths: list[str] = []

    for spec in catalog.templates:
        export_path = effective_output_root / spec.export_path
        if export_path.exists() and not force:
            raise FileExistsError(f"Refusing to replace existing SSH/devbox export at {export_path} without --force.")
    ...
    if export_catalog_path.exists() and not force:
        raise FileExistsError(
            f"Refusing to replace existing SSH/devbox catalog export at {export_catalog_path} without --force."
        )
