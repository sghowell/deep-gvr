...
| codex-local-backend | planned | Codex local is supported as a real orchestrator backend so the Codex path no longer requires Hermes underneath. | The runtime now has a typed backend-selection seam, but `runtime.orchestrator_backend=codex_local` still fails explicitly because the actual Codex-native transport is the next slice. | Codex-native orchestrator transport, preflight/release integration, and docs for the non-Hermes execution path. | [56-codex-local-backend.md](../plans/56-codex-local-backend.md) | A configured Codex-local backend can execute runs without Hermes installed, and Codex preflight/operator docs describe that path as a real backend rather than only a peer surface. |
