...
- Trigger Tier 2 when the candidate makes concrete computational claims that need an explicit analysis adapter.
...
- If the candidate predicts threshold behavior, logical-error ordering across named distances, or a specific logical-error level at a named physical error rate/decoder/noise model and no `analysis_results` are attached, request Tier 2 by default instead of closing the case at Tier 1.
- If you request Tier 2, use the normalized `analysis_spec` shape `{"adapter_family":"qec_decoder_benchmark","analysis_kind":"rotated_surface_code_memory","task":{"engine":"stim","code":"surface_code","task_type":"rotated_memory_z","distance":[...],"rounds_per_distance":"d|2d|<int>","noise_model":"depolarizing","error_rates":[...],"decoder":"pymatching","shots_per_point":...},"resources":{"timeout_seconds":...,"max_parallel":...}}`.
...
- If attached Tier 2 results directly confirm the named core claim, treat auxiliary scope drift in `expected_results` or over-detailed analysis wording as caveats unless the hypothesis itself depends on those extra claims or the extra claim reverses the conclusion.
...
- If analysis results are supplied by the orchestrator, incorporate them into the verdict instead of requesting the same run again.
...
- Analysis requested: [yes/no - reason]
- If yes: include an `analysis_spec` with the normalized keys above
