...
| codex-runtime-hardening | realized | Native Codex runs now preserve structured per-role response/error data in the standard transcript artifact and emit Codex-specific runtime evidence without conflating the Hermes-only plan-26 capability-closure probes. | `src/deep_gvr/orchestrator.py`, `src/deep_gvr/cli.py`, `tests/test_cli.py`, `plans/60-codex-runtime-hardening.md` |
