...
5. If Tier 3 is expected, inspect `verification.tier3.backend` in the runtime config first. For `backend: aristotle`, run `bash scripts/setup_mcp.sh --install --check` so `~/.hermes/config.yaml` has `mcp_servers.aristotle` and the local environment confirms `ARISTOTLE_API_KEY` plus Hermes MCP readiness. For `backend: mathcode`, ensure the configured local checkout exists and includes `AUTOLEAN/`, `lean-workspace/`, and an executable `run` wrapper.
